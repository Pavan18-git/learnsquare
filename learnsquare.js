...
document.getElementById('bars-icon').addEventListener('click', function () {
    ...
    // Toggle the 'active' class to show/hide the card
    hoverCard.classList.toggle('active');
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function toggleModules() {
        if (window.innerWidth <= 768) {
            ...
            // Laptop view: Show up to 9 features, hide the rest
            modules.forEach((module, index) => {
                if (index >= 9) {
                    module.classList.toggle('hidden');
                }
            ...
            // Desktop view: Show up to 9 features, hide the rest
            modules.forEach((module, index) => {
                if (index === 9) {
                    module.classList.toggle('hidden');
                }
        ...
        viewMoreBtn.textContent = isExpanded ? 'View More Features' : 'View Less';
        isExpanded = !isExpanded;
    }

    function initialSetup() {
        if (window.innerWidth <= 768) {
            // Mobile view: Hide all but first 3
            modules.forEach((module, index) => {
                if (index > 2) {
            ...
            // Laptop view: Show first 9, hide the rest
            modules.forEach((module, index) => {
                if (index < 9) {
                    module.classList.remove('hidden');
                } else {
            ...
            // Desktop view: Show first 9, hide the last one
            modules.forEach((module, index) => {
                if (index < 9) {
                    module.classList.remove('hidden');
                } else {
    ...
    viewMoreBtn.addEventListener('click', toggleModules);
});
